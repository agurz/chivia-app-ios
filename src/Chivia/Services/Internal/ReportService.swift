...
class ReportService {
    ...
    let REPORT_SERVICE_URL = "http://localhost:3000/report" // "https://chivia.herokuapp.com/report"
    
    func post(report: Report) -> Promise<Bool> {
        return Promise { fulfill, reject in
            Alamofire
                .request("\(REPORT_SERVICE_URL)", method: .post, parameters: report.toJSON().dictionaryObject, encoding: JSONEncoding.default)
                .validate()
